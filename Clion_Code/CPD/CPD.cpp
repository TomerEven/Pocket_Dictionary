...
void CPD::print_body_as_array() const {
    ...
    print_array_as_integers<uint32_t>(&(a[start]), end - start + 1);
}
...
void CPD::print_counters_as_array() const {
    ...
    print_array_as_integers<uint32_t>(&(a[temp_index]), size - temp_index);
}

void CPD::print_counters_as_consecutive_memory() const {
    auto temp_index = get_last_a_index_containing_the_body();
    print_array_as_consecutive_memory<uint32_t>(&(a[temp_index]), size - temp_index, cout);
}
