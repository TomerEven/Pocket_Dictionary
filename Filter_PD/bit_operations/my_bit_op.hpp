 ...
 * @param a
 * @param a_size
 * @return The index of the set bit in "pd" (were "pd" is pd vector of bits.)
 */
template<typename T>
 ...
 * @param bit_end_index
 * @return
 * In python syntax, assuming "A" is pd string:
...
//        cout << "H2" << endl;
        ...
        // pd[i] = (pd[i] << fp_size) | (pd[i + 1] >> (BODY_BLOCK_SIZE - fp_size));
//        cout << "H3" << endl;
 ...
 * @param a_size
 * @param k
 * @param start Stores result of SELECT(pd, k) + 1
 * @param end Stores result of SELECT(pd, k + 1)
 */
template<typename T>
...
*/
        ...
        /*else if (temp_counter == k) {
            *start = (i * slot_size) + (slot_size - __builtin_ctz(pd[i]) - 1);
            *end = find_first_set_bit<T>(&(pd[i + 1]), a_size - (i + 1)) + slot_size * (i + 1);
...
void update_element_with_fixed_size(T *a, size_t bit_start_pos, size_t bit_end_pos, T new_val, size_t a_size) {
    ...
    if (start_index == end_index) {
        ...
        temp |= left;
          ...
          //        uint32_t  temp3 = (((pd[start_index] >> first_shift) & (~MASK(length))) | new_val) << second_shift;
                    uint32_t temp2 = ((((pd[start_index] >> first_shift) << length) | new_val) << second_shift) | left;

        //Maybe: shift_second  + 1 ;
        uint32_t temp_mask = ~(MASK(first_shift) & MASK(second_shift));
        uint32_t temp4 = pd[start_index] & (temp_mask) | (new_val << second_shift);
...
//        assert(pd[start_index] == (left | right));
...
//    size_t new_val_length = new_end - new_start;
    ...
    auto prev_val = read_word<T>(pd, a_size, prev_start, prev_end);

    auto slot_size = sizeof(pd[0]) * CHAR_BIT;
    auto start_index = prev_start / slot_size;
    size_t shift = new_length_minus_prev;
    ...
    string var_names[num] = {"prev_val", "new_val",
                             ...
                             "pd[start_index]", "pd[start_index + 1]"};
 ...
 * @param start
 * @param end
 * @return With python syntax, assuming v is pd string of {0,1}*: return int(v[start:end],2)
 */
template<typename T>
 ...
 *      \*set in pd*\: [int(vec[i:(i+1)*word_size,2) for i in range(a_size)]
 */
template<typename T>
...
//template<typename T>
//void from_array_to_vector(vector<bool> *vec, const T *pd, size_t a_size);

/*

template<typename T>
void from_vector_to_array(const vector<bool> *vec, T *pd, size_t a_size);
*/
